...
def _frontpage_show(context, data_dict):
    if db.frontpage_table is None:
...
def _frontpage_list(context, data_dict):
    search = {}
    if db.frontpage_table is None:
        db.init_db(context['model'])
    org_id = data_dict.get('org_id')
    ...
    if not org_id:
        ...
        try:
            p.toolkit.check_access('ckanext_frontpage_update', context, data_dict)
            if not private:
...
def _frontpage_delete(context, data_dict):
    if db.frontpage_table is None:
        db.init_db(context['model'])
    org_id = data_dict.get('org_id')
    page = data_dict.get('page')
...
def _frontpage_update(context, data_dict):
    if db.frontpage_table is None:
        db.init_db(context['model'])
    org_id = data_dict.get('org_id')
    page = data_dict.get('page')
...
def frontpage_upload(context, data_dict):

    try:
        p.toolkit.check_access('ckanext_frontpage_upload', context, data_dict)
    except p.toolkit.NotAuthorized:
...
@tk.side_effect_free
def frontpage_show(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_frontpage_show', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_show(context, data_dict)


def frontpage_update(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_frontpage_update', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_update(context, data_dict)


def frontpage_delete(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_frontpage_delete', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_delete(context, data_dict)

@tk.side_effect_free
def frontpage_list(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_frontpage_list', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_list(context, data_dict)

@tk.side_effect_free
def org_frontpage_show(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_org_frontpage_show', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_show(context, data_dict)


def org_frontpage_update(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_org_frontpage_update', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_update(context, data_dict)


def org_frontpage_delete(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_org_frontpage_delete', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_delete(context, data_dict)

@tk.side_effect_free
def org_frontpage_list(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_org_frontpage_list', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_list(context, data_dict)

@tk.side_effect_free
def group_frontpage_show(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_group_frontpage_show', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_show(context, data_dict)


def group_frontpage_update(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_group_frontpage_update', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_update(context, data_dict)


def group_frontpage_delete(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_group_frontpage_delete', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_delete(context, data_dict)

@tk.side_effect_free
def group_frontpage_list(context, data_dict):
    try:
        p.toolkit.check_access('ckanext_group_frontpage_list', context, data_dict)
    except p.toolkit.NotAuthorized:
        p.toolkit.abort(401, p.toolkit._('Not authorized to see this page'))
    return _frontpage_list(context, data_dict)
